...
if(btnval.equalsIgnoreCase("save"))//to save the record
   {
	   ...
	   try
	   {
		  ...
		  // out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("insert into staff values(?,?,?,?,?)");
		   psmt.setString(1,t1);
		   psmt.setString(2,t2);
		   psmt.setString(3,t3);
		   psmt.setString(4,t4);
   ...
   if(btnval.equalsIgnoreCase("delete"))//to delete the record
   {
	   ...
	   try
	   {
		   ...
		   Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
		  // out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("delete from staff where staff_id=?");
		   psmt.setString(1,t1);
		   psmt.executeQuery();
   ...
   if(btnval.equalsIgnoreCase("update"))//to upadate the record
   {
	   ...
	   try
	   {
		   ...
		   Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
		  // out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("update staff set name=?,role=?,contact=?,shift=? where staff_id=?");
		   psmt.setString(1,t2);
		   psmt.setString(2,t3);
		   psmt.setString(3,t4);
		   psmt.setString(4,t5);
  	 ...
  	 try
  	  {
  		  ...
  		  ResultSet rs=smt.executeQuery("select * from staff"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  //out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from staff where lower(staff_id)='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  //out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from staff where lower("+col+")='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
