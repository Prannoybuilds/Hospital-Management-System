...
if(btnval.equalsIgnoreCase("save"))//to save the record
   ...
   if(btnval.equalsIgnoreCase("delete"))//to delete the record
   {
	   ...
	   try
	   {
		   ...
		   Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
		   out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("delete from doctor where doctor_id=?");
		   psmt.setString(1,t1);
		   psmt.executeQuery();
   ...
   if(btnval.equalsIgnoreCase("update"))//to upadate the record
   {
	   ...
	   try
	   {
		   ...
		   Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
		   out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("update doctor set name=?,specialization=?,contact=?,department=? where doctor_id=?");
		   psmt.setString(1,t2);
		   psmt.setString(2,t3);
		   psmt.setString(3,t4);
		   psmt.setString(4,t5);
  	 ...
  	 try
  	  {
  		  ...
  		  ResultSet rs=smt.executeQuery("select * from doctor"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from doctor where lower(doctor_id)='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from doctor where lower("+col+")='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
