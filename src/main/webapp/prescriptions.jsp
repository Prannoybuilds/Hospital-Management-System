...
if(btnval.equalsIgnoreCase("save"))//to save the record
   {
	   ...
	   try
	   {
		   ...
		   PreparedStatement psmt=conn.prepareStatement("insert into prescriptions values(?,?,?,?,?)");
		   psmt.setString(1,t1);
		   psmt.setString(2,t2);
		   psmt.setString(3,t3);
		   psmt.setString(4,t4);
   ...
   if(btnval.equalsIgnoreCase("update"))//to upadate the record
   {
	   ...
	   try
	   {
		   ...
		   Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
		   out.println("connected");
		   PreparedStatement psmt=conn.prepareStatement("update prescriptions set patient_id=?,doctor_id=?,medicine_id=?,dosage=? where prescription_id=?");
		   psmt.setString(1,t2);
		   psmt.setString(2,t3);
		   psmt.setString(3,t4);
		   psmt.setString(4,t5);
  	 ...
  	 try
  	  {
  		  ...
  		  ResultSet rs=smt.executeQuery("select * from prescriptions"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from prescriptions where lower(prescription_id)='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
  	 ...
  	 try
  	  {
  		  ...
  		  //out.println("select * from prescriptions where lower('"+col+"')='"+t1.toLowerCase()+"'");
  		  Class.forName("oracle.jdbc.driver.OracleDriver");
  		  Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hospital","hospital");
  		  //out.println("Connected");
  		  Statement smt=conn.createStatement();
  		  ResultSet rs=smt.executeQuery("select * from prescriptions where lower("+col+")='"+t1.toLowerCase()+"'"); // rs is the buffer
  		  %>
  		  <center>
  		  <table border=2>
  		  <tr>
